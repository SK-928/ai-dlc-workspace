...
3. requirements.md を作る — brief をテスト可能な要件に分解し、番号を振る(F1, F2, …)。成功条件は達成を確認する手順まで書く
4. design.md を作る — 構成と技術に加え、タスク間の共通契約(CLI・データ型・ファイル形式)まで書く。見た目が品質の成果物はモックを提示して承認する
5. tasks.md を作る — 1 サイクルで完了できる大きさの単位に割る。id(T1, T2, …)・依存・検証(手順と期待結果まで)を書く。見た目が品質のタスクはユーザーへの提示を検証に含める
6. 1 文書完成ごとに承認を仰ぎ、通ったら次へ進む。決定・承認・成果物の配置はそれぞれ `audit.md` に 1 行ずつ追記する
7. 決められない項目は、仮定・トリガー・期限を添えて記録する(ideation と同じ)。期限が来た懸案はその時点で解決し、残したまま次へ進まない

タスクの詳細設計は `construction/design-T<N>.md` に置き、タスク間の依存は design.md の共通契約のみ経由にする(1 タスクを別セッションで完結できる)。
