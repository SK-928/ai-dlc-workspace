...
3. テストする。自動テストを実行し、通らなければ直してから次へ。手動確認の場合は結果を記録する
4. tasks.md の当該タスクを完了に更新し、`audit.md` に 1 行追記する(実装とテスト結果)
5. 全タスクが完了するまで繰り返す
...
全タスク完了・テスト通過・report.md があること。次は `/aidlc-gate` で点検してから operation へ進む。
