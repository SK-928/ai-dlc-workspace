...
2. タスク設計を確定する — `construction/design-T<N>.md` を読み、不足あれば書く。共通契約の不足は inception へ戻す提案をする
3. 実装する。design.md とタスク設計に従い、そのタスクの分だけをやる(先回りして作らない)
4. 検証する。タスク設計の検証手順を実行し、通らなければ直してから次へ。手動確認・ユーザーへの提示はその結果を記録する
5. tasks.md の当該タスクを完了に更新し、`audit.md` に 1 行追記する(実装と検証結果)
6. 全タスクが完了するまで繰り返す
7. 完了後、`aidlc-docs/construction/report.md` に実装内容とテスト結果をまとめる
